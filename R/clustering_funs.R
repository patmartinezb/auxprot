...
#' @param hc An object of class "hclust" created with the `dend_calc()`
#'   function. 
#' @param clust Numeric vector of length 1, refers to the cluster number.
#'
#' @return A ggplot2 object.
#' 
#' @export cluster_plot
cluster_plot <- function(df, meta, hc, clust){
  
  if ((length(clust) > 1) | (length(clust) < 1)){
    stop("`clust` should be a numeric vector of length 1.")
  }
  
  if (is.null(clust) | (clust <= 0)){
    stop("`clust` should be a valid, positive number, and different from zero.")
  }
